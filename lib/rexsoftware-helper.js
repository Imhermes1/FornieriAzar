                    ...
                    activeListings.map(async (listing) => {
                        try {
                            const readResponse = await rexApiClient.request('/v1/rex/Listings/read', {
                                ...
                                body: JSON.stringify({
                                    id: listing.id,
                                    extra_fields: ['advert_internet', 'images', 'agent', 'listing_adverts', 'related', 'events', 'listing_events']
                                })
                            });
...
export async function getFilteredListings(filters = {}) {
    const {
        status = 'available',
        type,
        filter, // 'auctions' or 'inspections'
        minPrice,
        maxPrice,
        bedrooms,
        bathrooms,
    ...
    let filtered = allListings.filter(item => {
        // Special filter for auctions - show only properties with upcoming auctions
        if (filter === 'auctions') {
            if (!item.hasUpcomingAuction) return false;
            // Auctions filter implies available status
            if (item.status !== 'available') return false;
        }
        // Special filter for inspections - show only properties with upcoming inspections
        else if (filter === 'inspections') {
            if (!item.hasUpcomingInspection) return false;
            // Inspections filter implies available status
            if (item.status !== 'available') return false;
        }
        // Normal status filter (when not using special filters)
        else if (status !== 'all') {
            if (status === 'available') {
                if (item.status !== 'available') return false;
            } else {
    ...
    // Sort by auction date if filtering for auctions
    if (filter === 'auctions') {
        filtered.sort((a, b) => new Date(a.auctionDate) - new Date(b.auctionDate));
    }
    // Sort by next inspection date if filtering for inspections
    else if (filter === 'inspections') {
        filtered.sort((a, b) => {
            const aDate = a.inspections?.[0]?.date;
            const bDate = b.inspections?.[0]?.date;
...
export function normalizeRexProperty(p) {
    ...
    // Extract events (auctions, open homes/inspections)
    const allEvents = p.events || p.listing_events || p.related?.listing_events || [];
    const eventsArray = Array.isArray(allEvents) ? allEvents : [];

    // Find upcoming auction
    const now = new Date();
    const auctionEvent = eventsArray.find(e =>
        (e.event_type?.id === 'auction' || e.type?.id === 'auction' || e.event_type === 'auction') &&
        new Date(e.event_date || e.date || e.start_date) > now
    );

    // Find upcoming inspections/open homes
    const inspectionEvents = eventsArray
        .filter(e =>
            (e.event_type?.id === 'open_home' || e.type?.id === 'open_home' ||
                e.event_type === 'open_home' || e.event_type?.id === 'inspection') &&
            new Date(e.event_date || e.date || e.start_date) > now
        )
        .sort((a, b) => new Date(a.event_date || a.date || a.start_date) - new Date(b.event_date || b.date || b.start_date))
        .slice(0, 5) // Limit to next 5 inspections
        .map(e => ({
            date: e.event_date || e.date || e.start_date,
            startTime: e.start_time || e.time_start,
            endTime: e.end_time || e.time_end,
    ...
    return {
        ...
        // Events - Auctions and Inspections
        auctionDate: auctionEvent ? (auctionEvent.event_date || auctionEvent.date || auctionEvent.start_date) : null,
        auctionTime: auctionEvent ? (auctionEvent.start_time || auctionEvent.time_start) : null,
        auctionVenue: auctionEvent ? (auctionEvent.venue || auctionEvent.location) : null,
        hasUpcomingAuction: !!auctionEvent,

        inspections: inspectionEvents,
        hasUpcomingInspection: inspectionEvents.length > 0,
